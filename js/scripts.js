	                ...
	                $('body').waitForImages({
	                    finished: function() {
	                        ...
	                        setTimeout(function() {
	                            ...
	                            TweenMax.to($('.swiper-slide-active').find('.slider-hero-title'), 0.7, { force3D: true, y: 0, delay: 0.9, ease: Power2.easeOut });
	...
	function FirstLoad() {
	    ...
	    if ($("#slide3").length) {
	        var tube = new TubeAnimation("slide3", {
	            //Main settings
	            TUBE_SIZE: 200,
	            TUBE_WIDTH: 50,
	            TUBE_DISTANCE: 1000,
	            ...
	            TUBE_STRETCH: 200,
	            ROTATION_SPEED: 5,
	            DOT_COLOR: 'rgba(240, 240, 240, 0.05)',
	            DOT_SIZE: 2,
	            DETALIZATION: 209,
	            MESH_DISTRIBUTION: 2,
	            MESH_RATIO: 18.103,
	            ...
	            WAVE_1_MOTION_SPEED: 3,
	            WAVE_2_MOTION_SPEED: 2,
	            // WAVE_3_MOTION_SPEED: -3			
	        });
	    }
	    if ($("#slide2").length) {
	        var tube2 = new TubeAnimation("slide2", {
	            TUBE_SIZE: 243.162,
	            TUBE_WIDTH: 100,
	            TUBE_DISTANCE: 1100,
	            PERSPECTIVE_DISTORTION: 0.196,
	            TUBE_TORSION: 0.696,
	            TUBE_STRETCH: 123.529,
	            ROTATION_SPEED: 9.912,
	            DOT_COLOR: 'rgba(240, 240, 240, 0.1)',
	            DOT_SIZE: 2,
	            DETALIZATION: 381,
	            MESH_DISTRIBUTION: 1.529,
	            MESH_RATIO: 111.007,
	            SURFACE_INITIAL_DISTORTION: {
	                wave1: { amplitude: 200, frequency: 19, phase: 2.526 },
	                wave2: { amplitude: 200, frequency: 19, phase: 0.862 },
	                wave3: { amplitude: 200, frequency: 17, phase: 0.739 }
	            }
	        });
	    }
	    ...
	    if ($("#page-content").hasClass("light-content")) {
	        $("main, nav, #main-content").css('background-color', '#000000');
	        $('#magic-cursor').addClass('light-content');
	        if ($('#hero').length > 0) {
	            if ($('#hero').hasClass("has-image")) {
	...
	function Showcase() {
	    ...
	    if ($('#showcase-slider').length > 0) {
	        ...
	        var counter = [];
	        var paragraphs = [];
	        var slidersHeroTitle = [];
	        $('#showcase-slider .swiper-slide').each(function(i) {
	            titles.push($(this).data('title'));
	            subtitle.push($(this).data('subtitle'))
	            counter.push($(this).data('number'))
	            paragraphs.push($(this).data('paragraph'))
	            slidersHeroTitle.push($(this).data('herotitle'))
	        });
	        ...
	        var swiperOptions = {
	            ...
	            pagination: {
	                ...
	                renderBullet: function(index, className) {
	                    ...
	                    var outPut = '<div class="outer ' + className + ' ' + (index == 3 ? 'about-slider' : '') + '">';
	                    outPut += '<div class="inner">';

	                    outPut += '<div class="slider-hero-title">' + (slidersHeroTitle[index] ? slidersHeroTitle[index] : '') + '</div>';

	                    outPut += '<div class="subtitle">' + subtitle[index] + '</div>';
	                    outPut += '<div class="title paragraph-wrapper">' + titles[index];

	                    outPut += '<div class="inner-paragraph grey-color">' + paragraphs[index] + '</div>';

	                    outPut += '</div>'

	                    outPut += '<div class="counter-wrap">';
	                    outPut += '<div class="counter">' + counter[index] + '</div>';
	                    outPut += '</div>';
	                    outPut += '</div>';
	                    outPut += '</div>';
	                    return outPut;
	            ...
	            on: {
	                ...
	                slideNextTransitionStart: function() {
	                    if (swiper.realIndex != 0) {
	                        $('#home-page-logo-fixed').fadeOut()
	                        $('#home-page-first-animation').fadeOut()
	                    } else {
	                        $('#home-page-logo-fixed').fadeIn()
	                        $('#home-page-first-animation').fadeIn()
	                    }
	                    ...
	                    var prevslideHero = new TimelineLite();
	                    prevslideHero.staggerTo($('.swiper-pagination-bullet-active').prev().find('.slider-hero-title'), 0.4, { y: -60, opacity: 0, delay: 0.1, ease: Power2.easeInOut })
	                    ...
	                    var activeslideHero = new TimelineLite();
	                    activeslideHero.staggerTo($('.swiper-pagination-bullet-active').find('.slider-hero-title'), 0.5, { y: 0, opacity: 1, scale: 1, delay: 0.6, ease: Power2.easeOut })
	                    ...
	                    var nextslideHero = new TimelineLite();
	                    nextslideHero.staggerTo($('.swiper-pagination-bullet-active').next().find('.slider-hero-title'), 0.5, { y: 60, opacity: 0, ease: Power2.easeInOut })
	                ...
	                slidePrevTransitionStart: function() {
	                    ...
	                    var prevslideHero = new TimelineLite();
	                    prevslideHero.staggerTo($('.swiper-pagination-bullet-active').prev().find('.slider-hero-title'), 0.4, { y: -60, opacity: 0, delay: 0, ease: Power2.easeInOut })
	                    ...
	                    var activeslideHero = new TimelineLite();
	                    activeslideHero.staggerTo($('.swiper-pagination-bullet-active').find('.slider-hero-title'), 0.5, { y: 0, opacity: 1, scale: 1, delay: 0.4, ease: Power2.easeOut })
	                    ...
	                    var nextslideHero = new TimelineLite();
	                    nextslideHero.staggerTo($('.swiper-pagination-bullet-active').next().find('.slider-hero-title'), 0.5, { y: 60, opacity: 0, delay: 0, ease: Power2.easeInOut })
	                ...
	                slideChangeTransitionStart: function() {
	                    $('.hero-link').removeClass('hero-link-show');
	                    $('.swiper-slide-active .hero-link').addClass('hero-link-show');

	                    var currentSlideIndex = $('.swiper-slide-active .data-index').attr('data-index');

	                    if (currentSlideIndex == 2 && window.orientation == 90) {
	                        $('#showcase-holder').css('overflow', 'scroll');
	                    } else {
	                        $('#showcase-holder').css('overflow', 'hidden');
	                    }
	                    ...
	                    if (currentSlideIndex > 1) {
	                        ...
	                        if (currentSlideIndex < 2) {
	                            $('#social-links-responsive').fadeOut();
	                            setTimeout(function() {
	                            ...
	                            if ($(window).width() <= 479) {
	                                ...
	                                setTimeout(function() {
	                                    $('#social-links-responsive').fadeIn();
	                                }, 500);
	                            }
	                        }
	                    } else {
...
---------------------------------------------------*/
	...
	function ContactMap() {
	    ...
	    if (jQuery('#map_canvas').length > 0) {
	        var latlng = new google.maps.LatLng(25.1216111, 55.3773457);
	        var settings = {
	            zoom: 14,
	            center: new google.maps.LatLng(25.1216111, 55.3773457),
	        ...
	        var companyPos = new google.maps.LatLng(25.1216111, 55.3773457);
